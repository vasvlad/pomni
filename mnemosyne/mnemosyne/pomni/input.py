...
class EternalInputWidget(UiComponent):
    ...
    def __init__(self, component_manager):
        ...
        self.w_tree.signal_autoconnect(\
            dict([(sig, getattr(self, sig + "_cb")) \
                for sig in ["add_card", "add_card2", "input_to_main_menu"]]))
    ...
    def add_card_cb(self, widget):
        ...
        if self.update:
            # Update card
            self.ui_controller_main().update_related_cards(self.fact, 
                fact_data, self.card_type,
                [self.categories.get_child().get_text()], None)
        else:
            # Create new card
            self.ui_controller_main().create_new_cards(fact_data, 
                self.card_type, 0, [self.categories.get_child().get_text()])
    ...
    def clear_data_widgets(self):
        ...
        self.fields_container = self.create_fields_container()
        parent_fields_container.pack_start(self.fields_container,
            True, True, 0)


    def create_fields_container(self, fact = None):
    ...
    def activate(self, fact = None):
        ...
        parent_fields_container = \
                               self.w_tree.get_widget('fields_container_parent')
        self.fields_container = self.create_fields_container(self.fact)
